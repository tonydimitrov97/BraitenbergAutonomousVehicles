...
ArenaEntity* EntityFactory::CreateEntity(EntityType etype, Behavior behavior) {
  switch (etype) {
    case (kRobot):
      return CreateRobot(behavior);
      break;
    case (kLight):
      return CreateLight();
      break;
...
Robot* EntityFactory::CreateRobot(Behavior behavior) {
  auto* robot = new Robot(behavior);
  robot->set_type(kRobot);
  robot->set_color(ROBOT_COLOR);
  robot->set_pose(SetPoseRandomly());
  robot->set_radius(SetRobotSizeRandomly());
  ++entity_count_;
  ++robot_count_;
...
Light* EntityFactory::CreateLight() {
  ...
  light->set_radius(SetLightSizeRandomly());
  ++entity_count_;
  ++light_count_;
...
int EntityFactory::SetLightSizeRandomly() {
  return (random() % (LIGHT_MAX_RADIUS - LIGHT_MIN_RADIUS + 1) +
    LIGHT_MIN_RADIUS);
}

int EntityFactory::SetRobotSizeRandomly() {
  return (random() % (ROBOT_MAX_RADIUS - ROBOT_MIN_RADIUS + 1) +
    LIGHT_MIN_RADIUS);
}
